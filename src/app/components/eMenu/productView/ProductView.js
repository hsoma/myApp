  ...
  componentWillMount() {
    ...
    if (this.state.slectedProductInfo.is_rateable) {
      ...
      if(this.state.isPlan1Selected || this.state.isPlan2Selected || this.state.isPlan3Selected || this.state.isPlan4Selected){
        ...
        let credentialFlag;
        let providerRateUrl = `${config.emenuMobileGatewayAPI}/deal-jackets/${dealerData.dealjacketid}/deals/${currentSelectedProduct.dealer_id}/provider-product-rates/${currentSelectedProduct.provider_code}/${currentSelectedProduct.category_code}/`;
        ...
        let ratesData = {
          "dlr_cd": currentSelectedProduct.dealer_code,
          "deal_id": currentSelectedProduct.dealer_id,
          "deal_jacket_id": dealerData.dealjacketid,
          "category_cd": currentSelectedProduct.category_code,
          "provider_cd": currentSelectedProduct.provider_code,
          "product_rates_json": []
        }

        //let rateInfoLevel = getRateInfoLevel(pr[0],wholeLevel,p);
        //save Whole rates response
        axios.get(providerRateUrl,ratesData , axiosConfig).then((response) => {
          console.log('Slected Product Information FROM DB', response)
        }).catch((error) => {
          console.log('Error occurred while retrieving rates response FROM DB');
        });
